...
FROM elixir:1.15.1-slim AS dev
LABEL maintainer="Nick Janetakis <nick.janetakis@gmail.com>"
...
RUN apt-get update \
  && apt-get install -y --no-install-recommends build-essential curl inotify-tools mc nano \
  && rm -rf /var/lib/apt/lists/* /usr/share/doc /usr/share/man \
  && apt-get clean \
...
FROM elixir:1.15.1-slim AS prod
LABEL maintainer="Nick Janetakis <nick.janetakis@gmail.com>"
...
RUN apt-get update \
  && apt-get install -y --no-install-recommends curl mc nano \
  && rm -rf /var/lib/apt/lists/* /usr/share/doc /usr/share/man \
  && apt-get clean \
  && groupadd -g "${GID}" elixir \
  && useradd --create-home --no-log-init -u "${UID}" -g "${GID}" elixir \
